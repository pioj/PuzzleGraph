...
public class PuzzleTester : MonoBehaviour {
	...
	void RefreshData () {
		...
		puzzleFront.Reset ();
		if (layouter != null) layouter.Close ();
	...
	void MenuWindow (int id) {
		...
		if (menu == MenuOption.Menu) {
			...
			if (GUI.Button (GetRect (ref position, kButtonHeight, 0),
				PuzzleStateLayouter.UseThreading ? "Run Single Threaded" : "Run Multi Threaded", "MenuOption")) {
				PuzzleStateLayouter.UseThreading = !PuzzleStateLayouter.UseThreading;
			}
