...
Shader "Custom/GridShader" {
	...
	SubShader {
		...
		void surf (Input IN, inout SurfaceOutput o) {
			half4 ca = tex2D (_MainTex, IN.worldPos / unity_ObjectToWorld._m00 * 4 + float2 (-_Time.x*0.1, 0));
			half4 cb = tex2D (_MainTex, IN.worldPos / unity_ObjectToWorld._m00 * 4 + float2 (_Time.x*0.1, 0.5));
			half4 c = lerp (ca, cb, 0.4 + 0.2 * _SinTime.z);
			c = lerp (c, 1, 0.4);
			half4 d = tex2D (_GridTex, IN.worldPos / unity_ObjectToWorld._m00 * 32 );
