...
public class PuzzleStateLayouter {
	...
	public ReadOnlyPuzzle puzzle;

	Pointf[] vectors;
	Pointf[] oldVectors;
	double[,] distances;

	int extreme1Index, extreme2Index;
	Pointf centerTarget;
	double angleTarget;
	...
	public PuzzleStateLayouter (Puzzle puzzle) {
		this.puzzle = new ReadOnlyPuzzle (puzzle);
	...
	void UpdateInternal () {
		int n = puzzle.stateNodes.Length;
		for (int i = 0; i < n; i++)
	...
	void CalcGraphStats () {
		...
		extreme1 = puzzle.stateNodes[extreme1Index].point;
		extreme2 = puzzle.stateNodes[extreme2Index].point;
		// Give slight bias to existing found extreme to avoid unstable
		// stuttering when graph is equally long on multiple axes.
		extremeLength = (extreme2 - extreme1).magnitude * 1.01f;
